...
require "rabbiter/twitterstream"
require "oauth"
  ...
  class Client
    ...
    def setup
      ...
      @oauth_parameters = {
        ...
        :access => OAuth::Token.new(oauth_access_parameters[:access_token],
                                     oauth_access_parameters[:access_secret]),
      }
    end
    ...
    def start(*filters, &block)
      ...
      stream_options[:tls_verify] = false if Rabbit::Utils.windows?

      @connection = TwitterStream.new(@logger,
                                      @oauth_parameters,
                                      stream_options)

      @connection.track(filters) do |status|
